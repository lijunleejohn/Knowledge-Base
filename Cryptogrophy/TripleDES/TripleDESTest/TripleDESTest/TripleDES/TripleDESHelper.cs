...
namespace TripleDESTest.TripleDES
{
    public static class TripleDESHelper
    {
        ...
        {
            byte[] encrypted = TripleDESCore.EncryptByte2Byte(Encoding.UTF8.GetBytes(originalString), key, iv);
        ...
        {
            byte[] encrypted = TripleDESCore.EncryptByte2Byte(originalImage, key, iv);
            return Convert.ToBase64String(encrypted);
        }
        ...
        {
            byte[] decrypted = TripleDESCore.DecryptByte2Byte(Convert.FromBase64String(cryptedString), dKey, dIv);
