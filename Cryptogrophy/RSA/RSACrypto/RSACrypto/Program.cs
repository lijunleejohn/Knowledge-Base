...
class RSACrypto
{
    ...
    static void Main()
    {
        string clearText = "1234567890123456789012345678901234567890";
        try
        {
            ...
            byte[] dataToEncrypt = ByteConverter.GetBytes(clearText);
            byte[] encryptedData;
            byte[] decryptedData;
            ...
            {
                Console.WriteLine("Asymmetric encryption/decryption: everyone can encrypt (using a public key), only the host can decrypt (using a private key)");
