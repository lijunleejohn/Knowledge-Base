...
1. The following method is also added with the `Authorize` attribute with the name of the policy that enforces that the signed-in user is present in the app role **UserWriters**, that permits listing of roles and groups the signed-in user is assigned to.
